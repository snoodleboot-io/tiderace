#!/usr/bin/env bash
# Benchmark: pytest vs the tiderace engine (`tiderace-daemon`), over the same corpus. Honest
# cold-full-run + warm scenarios. The engine runs no-fork+restore by default; TIDERACE_FORCE_FORK=1
...
VENV="${2:-$R/.tiderace-fx-venv/bin/python}"
TIDERACE="$R/engine/target/release/tiderace-daemon"
export TIDERACE_PYTHON="$VENV" TIDERACE_SHIM="$R/engine/py-shim/shim.py"

[ -x "$TIDERACE" ] || { echo "missing $TIDERACE — build it first (see header)"; exit 1; }
...
# native runs no-fork+restore BY DEFAULT (no flag); TIDERACE_FORCE_FORK=1 is a debug knob for the baseline.
hyperfine --warmup 1 --runs 8 \
  --prepare "true"                      -n "pytest"                   "$VENV -m pytest -q ." \
  --prepare "rm -f .tiderace-state.json" -n "tiderace (default,no-fork)" "$TIDERACE run . --all" \
  --prepare "rm -f .tiderace-state.json" -n "tiderace (force-fork)"      "TIDERACE_FORCE_FORK=1 $TIDERACE run . --all"

echo
echo "### Scenario 2 — WARM, no changes (re-run after a clean run; impact analysis skips all)"
rm -f .tiderace-state.json; "$TIDERACE" run . >/dev/null 2>&1                  # populate native state
hyperfine --warmup 1 --runs 5 \
  -n "pytest (no warm mode)"    "$VENV -m pytest -q ." \
  -n "tiderace (impact-skip)"   "$TIDERACE run ."
...
echo "  tiderace (warm, 1 test):"; "$TIDERACE" bench "$D" 4 | sed 's/^/    /'
