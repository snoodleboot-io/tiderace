...
This directory holds a self-contained benchmark harness that compares **tiderace**
...
- `tiderace` binary at `target/debug/tiderace` (build with `cargo build` if missing).
- A Python venv at `.tiderace-bench-venv/` with `pytest`, `coverage`,
  `pytest-xdist`, and `pytest-testmon` installed.
- `hyperfine` (v1.x) on `PATH`.

Override paths with `--tiderace <path>` and `--python <interpreter>` if your
layout differs.
...
regime that exposes tiderace's subprocess-per-test cost most starkly, so it is the
...
regime favours parallel runners like `pytest-xdist` and tiderace's worker pool.
...
|----------|--------------------------|------------------|
| `tiderace_cold_full` | `tiderace --all --python <venv> tests/` | A cold full run with no skipping. `--prepare` runs `tiderace clear` before each timed run, so every iteration is genuinely cold. This is tiderace's **worst case** on fast tests. |
| `tiderace_warm_noop` | `tiderace --python <venv> tests/` | A warm impact run with **no source changes**. `--setup` primes the dependency graph once (`tiderace clear` then a coverage run); timed runs change nothing, so impact analysis should skip every test. tiderace's **best case**. |
| `tiderace_warm_one_module` | `tiderace --python <venv> tests/` | A warm impact run after editing **one** source module. `--setup` primes; `--prepare` restores the pristine module then touches it, so each run re-runs only the affected tests. tiderace's **typical edit-loop case**. |
...
| `pytest_testmon_cold` | `python -m pytest -q --testmon` | pytest-testmon with **no prior data** (`--prepare` deletes `.testmondata`). The fair cold comparison to tiderace's cold run. |
| `pytest_testmon_warm` | `python -m pytest -q --testmon` | pytest-testmon **primed**, no changes — testmon's own impact-skipping. The fair warm comparison to tiderace's warm run. |
| `unittest` | `python -m unittest discover -s tests -t .` | The stdlib runner, for reference. |

### Why priming uses coverage

tiderace builds its test→file dependency graph from coverage data. The state
database only records `test_deps` when a test reports covered files, which
requires running tiderace with `-c/--coverage`. So the warm scenarios prime with
`tiderace -c ... tests/` before the timed (non-coverage) impact runs.

## The honest cold-vs-warm story

tiderace runs each test in its **own** `python -m pytest <nodeid>` subprocess (see
**ADR-001** in `docs/design/decisions.md`). Interpreter startup is ~250 ms per
test, so:

- On a **cold full run of many fast tests**, tiderace is **slower** than
  in-process pytest — it pays startup once per test instead of once per session.
  We measure and report this; it is not hidden.
- On a **warm run**, tiderace hashes sources, consults its dependency graph, and
  re-runs only the tests affected by a change — skipping everything else. This is
  where the subprocess cost is amortised away and tiderace wins, especially on
  large suites where a small edit touches few tests.

Reporting **both** regimes — and comparing tiderace warm against pytest-testmon
warm, not just against full pytest — is what makes this benchmark fair rather
than cherry-picked.

> **Note (June 2026):** tiderace's warm-skip path is being hardened on Lane A
