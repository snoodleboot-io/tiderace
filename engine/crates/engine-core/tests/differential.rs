...
//! venv with pytest (the fx venv, else the Phase-1 `.tiderace-spike-venv`); absent one it skips via
//! `engine_core::testing::skip_live`, which fails instead under `TIDERACE_REQUIRE_LIVE=1`.
...
/// rest of the live suite provision) and falls back to the Phase-1 Lane-0 `.tiderace-spike-venv`.
///
...
fn venv_python() -> Option<PathBuf> {
    [
        ".tiderace-fx-venv/bin/python",
        ".tiderace-spike-venv/bin/python",
    ]
    .iter()
...
fn engine_outcomes_match_pytest_across_three_styles() {
    let Some(python) = venv_python() else {
        skip_live(
            "no venv with pytest (`.tiderace-fx-venv` / `.tiderace-spike-venv`) — provision one",
        );
        return;
    };

    // Isolated corpus in a temp dir.
    let dir = std::env::temp_dir().join(format!("tiderace_diff_{}", std::process::id()));
