...
| **L0-3** | Isolated CPython venv (3.12+) | The interpreter the Wellspring spawns; **isolated** so the engine never touches system Python | `uv venv .tiderace-spike-venv` (`uv` already present; mirrors [prior phase E2](../../../completed/phase-1-hardening-benchmarks/env-manifest.md)) | `./.tiderace-spike-venv/bin/python -V` → 3.12+ | — |
