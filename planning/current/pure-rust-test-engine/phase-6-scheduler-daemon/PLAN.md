...
| W9 | `[tool.riptide.scheduler]` config (`scheduler`/`split_threshold`/`min_group_split`/`worker_count`/`history_decay`/`cold_proxy`), `WorkerCaps`-aware tuning | [06 §7](../design/06-scheduler.md), [13](../design/13-cross-cutting.md) | SCHED |
...
| W13 | `EventStream` + server-pushed notifications: `riptide/runStarted` · `riptide/result` · `riptide/runFinished` · `riptide/invalidated` · `riptide/collectionChanged` (adapts the engine `HookHost` stream, not a parallel one) | [08 §9.2–9.3](../design/08-daemon.md), [12](../design/12-plugin-host.md) | DAEMON |
...
| W18 | `LocalSocket` (`socket.rs`) — per-user/per-project, `$XDG_RUNTIME_DIR/riptide/`, `0700`/`0600`, `SO_PEERCRED` uid check, `ProjectKey`-derived path | [08 §6](../design/08-daemon.md) | DAEMON/SEC |
...
| E7 | **Invalidation corpus — edit *config*** — `pyproject.toml` `[tool.riptide]` change | generator | a config edit must trigger a **recycle** |
...
|--------------|---------|-----------|
| `riptide/runStarted` | `{ run_id, total_selected, impacted, cache_candidates }` | a run is admitted (progress denominator) |
| `riptide/result` | `{ run_id, TestResult }` (`served_from_cache`, `Outcome`, `Duration`, `RichDiff` on fail) | each test resolves — cache-served or freshly forked |
| `riptide/runFinished` | `{ run_id, RunReport }` | the run completes |
| `riptide/invalidated` | `{ files, kind, recycled: bool }` | the watcher invalidated state (and whether a recycle happened) |
| `riptide/collectionChanged` | `{ added, removed, collection_revision }` | a rescan changed the test set |
...
results** (`riptide/runStarted` → `riptide/result` ×N in completion order → `riptide/runFinished`).
...
forks only the impacted non-cached tests, and the daemon streams `riptide/invalidated{recycled:false}`
then `riptide/result` ×N. **Verification (test-agent + performance-agent, live):** the impacted set
...
`riptide/invalidated{recycled:true}`. **Verification (test-agent, live, the classic-failure-mode test):**
...
| A3 | `runImpacted` after a source edit re-runs exactly the impacted set; rest cache-served; `riptide/invalidated{recycled:false}` | impacted set vs E5 baseline |
| A4 | **conftest** edit (E6) ⇒ **recycle**; the next run reflects the changed fixture (no stale interpreter) | `riptide/invalidated{recycled:true}` + value reflected |
...
| A10 | `discover`/`subscribeResults` stream `riptide/result` in completion order, `run_id`-correlated, over the live socket | fake-IDE harness (E3) |
...
|--------|--------|--------|
| P1 | **Edit→result well under 100ms** on a warm daemon, small blast radius (the G4 thesis, [08 §1](../design/08-daemon.md)) | hyperfine; FS-save → first `riptide/result`; budget-decomposed (debounce + diff/invalidate + impact + cache-check + fork+body) |
...
|---|---------|------------------------|----------|
| S1 | **Cross-user access** to the warm interpreter / cache | `SO_PEERCRED` uid check; socket under `$XDG_RUNTIME_DIR/riptide/` `0700`/`0600`, owned by invoking uid; refuse any uid ≠ owner ([08 §6](../design/08-daemon.md)) | — |
...
server-pushed notifications (completion-order, run_id-correlated):
  riptide/runStarted     { run_id, total_selected, impacted, cache_candidates }
  riptide/result         { run_id, TestResult }   (served_from_cache, Outcome, Duration, RichDiff on fail)
  riptide/runFinished    { run_id, RunReport }
  riptide/invalidated    { files, kind, recycled: bool }
  riptide/collectionChanged { added, removed, collection_revision }
```
...
- Config: `[tool.riptide.scheduler]` — `scheduler` · `split_threshold`(1.5) · `min_group_split`(2) ·
