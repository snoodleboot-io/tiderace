...
`riptide watch` keeps a pool of **warm** Python workers running and re-runs only the
...
1. On startup, riptide collects your tests and spawns `-n` warm workers (default: CPU count),
   each of which imports pytest once.
2. It runs an initial pass to establish a baseline, then watches the tree.
3. When you save a file, riptide hashes the change, runs [impact analysis](../design/impact-analysis.md)
...
riptide watch tests/ -n 12 --python .venv/bin/python

# Watch specific paths
riptide watch tests/unit tests/integration
```
...
- **With a coverage graph** (you have run `riptide --coverage` at least once), editing a
  source file re-runs only the tests whose recorded dependencies changed.
- **Without one**, riptide is conservative: editing a *source* file re-runs every test that
...
```bash
riptide --coverage tests/      # build the per-test dependency graph
riptide watch tests/           # now source edits re-run only impacted tests
```
