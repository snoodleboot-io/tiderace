...
persisting `.tiderace-state.json` between runs.** A fresh CI checkout has no state, so without a cache
...
|---|---|---|---|
| **Safe** | `tiderace-daemon run <tests> --all` | Full parallel run of the whole suite **every time**. No skipping, no dependence on prior state. | Protected branches, release pipelines, anywhere correctness must not depend on a cache. |
| **Fast** | `tiderace-daemon run <tests>` (with cached state) | Impact-aware: re-runs only tests whose deps changed since the cached run; coverage kept fresh automatically. | PR / feature CI where you want speed and accept that selection is only as good as the cached graph. |
...
Build the engine from source (until prebuilt binaries are published), cache `.tiderace-state.json`,
and run:
  ...
  test:
    ...
    steps:
      ...
      - name: Configure the engine
        run: |
          echo "TIDERACE_SHIM=$PWD/engine/py-shim/shim.py" >> "$GITHUB_ENV"
          echo "TIDERACE_PYTHON=$(which python)"           >> "$GITHUB_ENV"

      # THE important bit for fast mode: persist impact-analysis state across runs.
      - uses: actions/cache@v4
        with:
          path: .tiderace-state.json
      ...
      - run: ./engine/target/release/tiderace-daemon run tests/
```

For **safe mode**, swap the last step for a forced full run (and you can drop the cache step):

```yaml
      - run: ./engine/target/release/tiderace-daemon run tests/ --all
```
...
- `.tiderace-state.json` is small (per-test deps + content hashes). It's safe to cache: a stale or
...
- **Cache** `.tiderace-state.json` between pipeline runs (GitLab `cache:`, CircleCI
  `save_cache` / `restore_cache`, etc.).
- **Run** `tiderace-daemon run tests/` (fast) or `tiderace-daemon run tests/ --all` (safe).
