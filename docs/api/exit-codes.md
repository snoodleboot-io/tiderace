...
| `1` | Test failure **or** runtime error — one or more tests failed/errored, or an internal error (collection failure, wellspring launch failure, missing `RIPTIDE_SHIM`, watch/serve I/O error) | both binaries |
...
- **Test failure and internal error share code `1`.** Neither binary distinguishes a failing test
  ...
  stderr. The `riptide run` pytest-style code comes from `RunReport::exit_code()` (`0` unless any
  outcome is a failure).
- **`64`** is the conventional `EX_USAGE` value, returned for argument/usage problems (e.g.
  `riptide` with fewer than two args, an unknown `riptide-daemon` mode) and for `riptide-daemon serve`
  on a platform without Unix sockets.
- A **missing `RIPTIDE_SHIM`** is treated as a runtime error and exits `1` (not `64`).
